...
class CarActuator(object):
    ...
    def __init__(self):
        ...
        self.client = rospy.ServiceProxy("mission",DestinationMsg)
    ...
    def actuator_ask_newtarget(self):
        self.request.request_type=0            #请求包编号为“请求新任务”
        ...
        rospy.loginfo("Get:%d,Send:%d",self.response.drug_location,self.response.deliver_destination)
        if(self.response.drug_location !=-1 and self.response.deliver_destination !=-1): #不是负-1代表请求成功
            self.status = 2 
        else:
            self.status = 3
    ...
    def actuator_updateABC(self):
        self.request.request_type=1            #请求包编号为“完成取药”
        self.client.call(self.request.car_no,self.request.request_type)
       
    #向服务器上报已送药
    def actuator_update1234(self):
        self.request.request_type=2            #请求包编号为“完成送药”
        self.client.call(self.request.car_no,self.request.request_type)

    #向服务器上报已到达手写数字识别区
    def actuator_arriveNum(self):
        self.request.request_type=3            #请求包编号为“到达手写数字识别区”
        self.client.call(self.request.car_no,self.request.request_type)
